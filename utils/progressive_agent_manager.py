...
class ProgressiveAgentManager:
    def __init__(self):
        ...
        # Base stage definitions - will be customized per target type
    ...
    def _get_stage_definitions(self, target_type: str) -> Dict[str, Dict[str, Any]]:
        """Get stage definitions customized for target type"""
    ...
    def create_progressive_agent(self, query: str, hours_old: int = 24, custom_tags: Optional[List[str]] = None, target_type: str = "hiring_managers", company_size: str = "all", location_filter: Optional[str] = None) -> ProgressiveAgent:
        ...
        # Get customized stage definitions for target type
        stage_definitions = self._get_stage_definitions(target_type)
        
        # Initialize stages
        stages = {}
        for stage_key, stage_config in stage_definitions.items():
    ...
    def _calculate_total_progress(self, agent_id: str):
        ...
        # Get the correct stage definitions for this agent's target type
        stage_definitions = self._get_stage_definitions(agent.target_type or "hiring_managers")
        
        for stage_key, stage in agent.stages.items():
            if stage_key in stage_definitions:
                weight = stage_definitions[stage_key]["weight"]
        ...
        if agent.total_progress == 100:
            ...
            # Check if critical stages failed based on target type
            if agent.target_type == "job_candidates":
                critical_stages = ["candidate_search"]  # Only candidate search is critical for job_candidates
            else:
                critical_stages = ["linkedin_fetch"]  # Only LinkedIn stage is truly critical for hiring_managers
            ...
            if agent.target_type == "job_candidates":
                total_results = agent.staged_results.total_contacts  # For candidates, count contacts
            else:
                total_results = sum(stage.results_count for stage in agent.stages.values() if stage.results_count)  # For hiring managers, count jobs
            
            if critical_failed and total_results == 0:
                ...
                # Check what stage is currently running
                if agent.target_type == "job_candidates":
                    if agent.stages.get("candidate_search", {}).status in ["completed", "failed"]:
                        agent.status = "enrichment_stage"
                    else:
                        agent.status = "candidate_stage"
                else:
                    if agent.stages.get("linkedin_fetch", {}).status in ["completed", "failed"]:
                        agent.status = "enrichment_stage"
                    else:
    ...
    def get_agent(self, agent_id: str) -> Optional[ProgressiveAgent]:
        """Get an agent by ID, checking memory first then database"""
        # First check in-memory cache
        agent = self.active_agents.get(agent_id)
        
        if agent:
            # For completed agents, refresh totals from database to ensure accuracy
            if agent.status == "completed":
                try:
                    # Get actual counts from database
                    total_contacts = progressive_agent_db.get_contact_count(agent_id)
                    total_campaigns = progressive_agent_db.get_campaign_count(agent_id)
                    total_jobs = progressive_agent_db.get_job_count(agent_id)
                    
                    # Update the agent's staged results with real database counts
                    agent.staged_results.total_contacts = total_contacts
                    agent.staged_results.total_campaigns = total_campaigns  
                    agent.staged_results.total_jobs = total_jobs
                    
                    logger.info(f"🔄 Refreshed agent {agent_id} totals: {total_contacts} contacts, {total_campaigns} campaigns, {total_jobs} jobs")
                except Exception as e:
                    logger.warning(f"Failed to refresh totals for agent {agent_id}: {e}")
    ...
    def finalize_agent(self, agent_id: str, final_stats: Dict):
        ...
        if agent_id in self.active_agents:
            ...
            # Refresh totals from database before finalizing to ensure accuracy
            try:
                total_contacts = progressive_agent_db.get_contact_count(agent_id)
                total_campaigns = progressive_agent_db.get_campaign_count(agent_id)
                total_jobs = progressive_agent_db.get_job_count(agent_id)
                ...
                logger.info(f"🔄 Pre-finalization refresh for agent {agent_id}: {total_contacts} contacts, {total_campaigns} campaigns, {total_jobs} jobs")
            except Exception as e:
                logger.warning(f"Failed to refresh totals before finalization for agent {agent_id}: {e}")
            ...
            # Update in database with current staged results
            asyncio.create_task(progressive_agent_db.save_agent_metadata(
                agent_id=agent_id,
                query=agent.query,
                status="completed",
