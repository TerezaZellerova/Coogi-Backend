"""
BULLETPROOF CAMPAIGN CREATOR - AWS SES Direct Integration
Creates and manages email campaigns using AWS SES as primary engine with Instantly.ai backup
"""
import os
import logging
import asyncio
...
class BulletproofCampaignCreator:
    def __init__(self):
        ...
        # Initialize AWS SES as primary email engine
        self.ses_service = AWSSESService()
        
        # Campaign service configuration (Instantly as backup only)
        self.backup_service = {
            "instantly": {
...
P.S. This is a confidential search, so your current employer won't be contacted without your explicit permission."""
    ...
    async def _create_company_campaign(self, 
        ...
        # Strategy 1: AWS SES Direct (Primary - 50K emails/day, 14 emails/sec)
        try:
            ses_campaign = await self._create_ses_campaign(campaign_content, contacts)
            if ses_campaign:
                logger.info(f"✅ AWS SES campaign created for {company}")
                # Ensure platform field is correctly set for SES
                ses_campaign["platform"] = "aws_ses"
                ses_campaign["service"] = "aws_ses"
                return ses_campaign
        except Exception as e:
            logger.error(f"❌ AWS SES failed for {company}: {e}")
        
        # Strategy 2: Instantly.ai (Backup for personalized drip campaigns)
        try:
            instantly_campaign = await self._create_instantly_campaign(campaign_content, contacts)
            if instantly_campaign:
                logger.info(f"✅ Instantly.ai backup campaign created for {company}")
                # Ensure platform field is correctly set for Instantly
                instantly_campaign["platform"] = "instantly"
                instantly_campaign["service"] = "instantly"
                return instantly_campaign
        except Exception as e:
            logger.error(f"❌ Instantly.ai backup failed for {company}: {e}")
        
        # Strategy 3: Internal campaign (final fallback)
        internal_campaign = self._create_internal_campaign(campaign_content, contacts)
        logger.info(f"✅ Internal fallback campaign created for {company}")
        # Ensure platform field is correctly set for internal
        internal_campaign["platform"] = "internal"
        internal_campaign["service"] = "internal"
    ...
    async def _generate_campaign_content(self, 
        ...
        return {
            ...
            "sender_info": sender_info,
            "campaign_type": campaign_type,
            "agent_id": sender_info.get("agent_id", "")  # Pass through agent_id if provided
        }
    ...
    async def _create_ses_campaign(self, content: Dict, contacts: List[Dict]) -> Optional[Dict]:
        """Create campaign using AWS SES Direct - Primary Email Engine (50K emails/day)"""
        if not self.ses_service or not self.ses_service.ses_client:
            logger.warning("⚠️ AWS SES not configured or unavailable")
            return None
        ...
        try:
            ...
            if not valid_contacts:
                logger.warning(f"⚠️ No valid email contacts for SES campaign: {campaign_name}")
                return None
            
            logger.info(f"🚀 Creating AWS SES campaign: {campaign_name} with {len(valid_contacts)} leads")
            
            # For AWS SES, we create the campaign structure but don't send immediately
            # This allows for batch processing, scheduling, and rate limiting
            campaign_id = f"ses_{random.randint(10000, 99999)}"
            
            # Create campaign record (would normally store in database)
            ses_campaign = {
                "id": campaign_id,
                "name": campaign_name,
                "description": f"AWS SES Direct campaign for {content['job']['company']}",
                "status": "ready",  # Ready to send via SES
                "service": "aws_ses",
                "platform": "aws_ses",
                "contacts": valid_contacts,
                "subject": subject,
                "message": message,
                "lead_count": len(valid_contacts),
                "leads_count": len(valid_contacts),
                "target_count": len(valid_contacts),
                "sent_count": 0,
                "delivered_count": 0,
                "open_count": 0,
                "click_count": 0,
                ...
                "is_demo": False,
                "ses_quota_used": len(valid_contacts),  # Track SES quota usage
                "estimated_send_time": f"{len(valid_contacts)/14:.1f} seconds",  # 14 emails/sec rate
                "ses_region": self.ses_service.region,
                "from_email": self.ses_service.from_email
            }
            
            logger.info(f"✅ AWS SES campaign created: {campaign_id}")
            logger.info(f"📊 Campaign will use {len(valid_contacts)} of your 50,000 daily SES quota")
            logger.info(f"⏱️ Estimated send time: {len(valid_contacts)/14:.1f} seconds at 14 emails/sec")
            
            return ses_campaign
            
        except Exception as e:
            logger.error(f"❌ AWS SES campaign creation error: {e}")
            return None
